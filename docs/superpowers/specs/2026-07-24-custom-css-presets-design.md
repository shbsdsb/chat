...
三个设置抽屉（API / 参数预设 / 自定义CSS）共用同一个 `SettingsDrawer` 外壳组件，App.vue 通过单一 `activeDrawer` 状态实现互斥：

```js
// 状态：null | 'api' | 'presets' | 'css'
...
内容通过 `<Transition mode="out-in">` + `v-if`/`v-else-if` 实现切换动画。
...
| `drawer-width-easing` | ease | 抽屉面板 |
| `drawer-slide-duration` | 0.2s | 内容页切换滑入滑出 |
| `drawer-slide-easing` | ease | 内容页 |
| `drawer-slide-enter-x` | 60px | 新内容从右侧滑入距离 |
| `drawer-slide-leave-x` | -40px | 旧内容向左滑出距离 |
| `title-fade-duration` | 0.15s | 标题文字切换淡入淡出 |
| `title-fade-easing` | ease | 标题文字 |
...
切换（API → 预设）：
  drawer 保持展开
  ├─ 标题 fade-out 0.15s → 更新文本 → fade-in 0.15s
  └─ 内容 slideLeft 40px + fadeOut (0.2s)
       └─ 完成后 →
          新内容 slideFromRight 60px + fadeIn (0.2s)
  总耗时 ≈ 0.4s
...
打开（首次点按钮）：
  drawer width→设定值 (0.2s)
  内容 slideFromRight 60px + fadeIn (0.2s)
```

### 5.4 代码位置

- 抽屉外壳：`SettingsDrawer.vue`，使用 `drawer.css`（`transition: width 0.2s ease`）
- 切换逻辑：`App.vue` — `activeDrawer` + `<Transition name="drawer-slide">` + `<Transition name="title-fade">`
- 动画 CSS：`App.vue` 全局 `<style>` 块
