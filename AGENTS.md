...
# 后端测试
cd backend
python -m pytest                           # 自动使用 tmp_path 隔离数据，39 tests
```
...
│   └── tests/                             # pytest（conftest.py monkeypatch DATA_DIR），6 个测试文件
├── frontend/
...
│       │   └── settings.js                # 设置 API
│       ├── composables/
│       │   └── useMarkdown.js             # MD 流式渲染：分段冻结策略、markdown-it + DOMPurify + hljs
│       ├── stores/
│       │   ├── chat.js                    # Pinia — 会话/消息/流式状态、lastMessageAt 即时排序
│       │   ├── settings.js                # Pinia — API 预设（多配置切换）
│       │   └── alert.js                   # Pinia — 全局弹窗提示
│       ├── components/                    # ConversationItem(编辑/删除按钮+弹窗), ConversationsDrawer(左→右可拖拽),
│       │                                  #   InputBar, MessageBubble(MD渲染+代码块复制), MessageList,
│       │                                  #   AlertDialog, ModelSelector, PresetSelector,
│       │                                  #   SettingsDrawer(右→左可拖拽), MessageActions,
│       │                                  #   HtmlPreview(HTML 代码/预览切换), ResponseFormatInput,
│       │                                  #   WelcomeBanner
│       ├── views/
